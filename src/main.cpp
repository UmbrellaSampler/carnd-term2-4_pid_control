...
int main() {
    ...
    // Values found by manual tuning
    PID pid_steering;
    pid_steering.Init(0.15, 0.0, 3.0);

    // PID controller for throttle. Decreases speed when cte is high
    // Values found by manual tuning
    PID pid_throttle;
    pid_throttle.Init(0.15, 0.0, 3.0);
        ...
        if (length && length > 2 && data[0] == '4' && data[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    throttle = 0.3 - pid_throttle.TotalError();
